...
#     return df
def convert_and_replace(df, replace_map, dtype_map=None):
    """
    Convert columns to specified data types and replace values. 

    Parameters:
    - df: pandas DataFrame
    - cols: list of column names to process
    - replace_map: dict {column_name: {old_value: new_value}} specifying value replacements (optional)

    Updates df in place.
    """
    rtn=df.replace(replace_map, inplace=False)
    for col in 
